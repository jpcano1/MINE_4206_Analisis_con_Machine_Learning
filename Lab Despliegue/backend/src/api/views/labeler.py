...
from ..models import Labeler as LabelerModel, LabelerSchema
    ...
    def post(self):
        ...
        try:
            ...
            return response_with(
                resp.SUCCESS_201,
                value={"message": "Your predictions were submitted successfully"},
            )
        except Exception:
            return response_with(resp.BAD_REQUEST_400, error="You have to send text")

    def get(self):
        fetched = LabelerModel.query.all()
        labeler_schema = LabelerSchema(many=True)
        labels = labeler_schema.dump(fetched)
        return response_with(resp.SUCCESS_200, value={"labels": labels})
